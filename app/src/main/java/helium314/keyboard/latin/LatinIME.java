...
public class LatinIME extends InputMethodService implements
    ...
    void onStartInputViewInternal(final EditorInfo editorInfo, final boolean restarting) {
        ...
        // Record typing history session start
        try {
            TypingHistoryRecorder recorder = TypingHistoryRecorder.getInstance((Context) this);
            recorder.onStartInputView(editorInfo, restarting);
        } catch (Exception e) {
            Log.e(TAG, "Error recording typing history session start", e);
        }
    ...
    void onFinishInputViewInternal(final boolean finishingInput) {
        ...
        // Record typing history session end
        try {
            TypingHistoryRecorder recorder = TypingHistoryRecorder.getInstance((Context) this);
            recorder.onFinishInputView();
        } catch (Exception e) {
            Log.e(TAG, "Error recording typing history session end", e);
        }
    ...
    public void onUpdateSelection(final int oldSelStart, final int oldSelEnd,
        ...
        // Notify TypingHistoryRecorder if input field was cleared (e.g. message sent or field reset).
        // Multi-signal detection: works across ALL apps (WhatsApp, Telegram, Gmail, browsers, etc.)
        if (newSelStart == 0 && newSelEnd == 0) {
            try {
                final TypingHistoryRecorder recorder = TypingHistoryRecorder.getInstance(getApplicationContext());
                if (recorder.hasActivityInSession()) {
                    // Confirm field is truly empty (not just cursor moved to start)
                    final InputConnection ic = getCurrentInputConnection();
                    if (ic != null) {
                        final CharSequence before = ic.getTextBeforeCursor(1, 0);
                        final CharSequence after = ic.getTextAfterCursor(1, 0);
                        final boolean fieldIsEmpty = (before == null || before.length() == 0)
                ...
                Log.e(TAG, "Error notifying typing history on field clear in onUpdateSelection", e);
            }
        }
