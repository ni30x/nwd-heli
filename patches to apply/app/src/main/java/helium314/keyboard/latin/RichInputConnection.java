...
public final class RichInputConnection implements PrivateCommandPerformer {
    ...
    public void commitText(final CharSequence text, final int newCursorPosition) {
        ...
        // Record typing history BEFORE committing
        recordTypingHistory(text.toString());
    ...
    /**
     * Records typing history for the committed text.
     * @param text The text that was committed.
     */
    private void recordTypingHistory(String text) {
        if (TextUtils.isEmpty(text)) return;

        try {
            TypingHistoryRecorder recorder = TypingHistoryRecorder.getInstance((Context) mParent);
            if (mParent instanceof LatinIME) {
                LatinIME latinIme = (LatinIME) mParent;
                EditorInfo editorInfo = mParent.getCurrentInputEditorInfo();
                if (editorInfo != null) {
                    String packageName = editorInfo.packageName;
                    int inputType = editorInfo.inputType;
                    recorder.onTextCommitted(text, packageName, inputType);
                }
            }
        } catch (Exception e) {
            Log.e(TAG, "Error recording typing history", e);
        }
    }
    ...
    public void deleteTextBeforeCursor(final int beforeLength) {
        ...
        // Record delete event BEFORE deletion - capture the exact characters being
        // removed from our local buffers (no extra IPC call to the target app needed).
        final String deletedText = computeDeletedTextForHistory(beforeLength);
        recordDeleteHistory(beforeLength, deletedText);
    ...
    /**
     * Computes the exact substring that is about to be removed by a
     * deleteTextBeforeCursor(beforeLength) call, using only the locally-tracked
     * "committed + composing" buffers (no IPC round-trip to the target app).
     * Must be called BEFORE those buffers are mutated.
     *
     * @param beforeLength Number of characters about to be deleted before the cursor.
     * @return The exact deleted text, or null if it could not be determined.
     */
    @Nullable
    private String computeDeletedTextForHistory(final int beforeLength) {
        if (beforeLength <= 0) return null;
        // mCommittedTextBeforeComposingText + mComposingText mirrors exactly what
        // deleteTextBeforeCursor() is about to trim from, so re-use it directly.
        final StringBuilder combined = new StringBuilder(mCommittedTextBeforeComposingText);
        combined.append(mComposingText);
        final int totalLen = combined.length();
        if (totalLen <= 0) return null;
        final int start = Math.max(totalLen - beforeLength, 0);
        return combined.substring(start, totalLen);
    }

    /**
     * Records delete/backspace history event.
     * @param charCount Number of characters being deleted.
     * @param deletedText The exact characters being deleted (may be null if unknown,
     *                     e.g. for legacy call sites), in which case the viewer falls
     *                     back to showing just the character count.
     */
    private void recordDeleteHistory(int charCount, @Nullable String deletedText) {
        ...
        try {
            ...
            if (mParent instanceof LatinIME) {
                ...
                if (editorInfo != null) {
                    String packageName = editorInfo.packageName;
                    int inputType = editorInfo.inputType;
                    recorder.onDeleteEvent(deletedText, charCount, packageName, inputType);
                }
            }
        } catch (Exception e) {
            Log.e(TAG, "Error recording delete history", e);
        }
    }
    ...
    public void performEditorAction(final int actionId) {
        ...
        if (isConnected()) {
            ...
            // Only notify typing history on send-like actions, not search/next/etc.
            if (actionId == EditorInfo.IME_ACTION_SEND
                ...
                try {
                    ...
                    final TypingHistoryRecorder recorder = TypingHistoryRecorder.getInstance(context);
                    final EditorInfo info = mParent.getCurrentInputEditorInfo();
                    final String packageName = info != null ? info.packageName : null;
                    final int inputType = info != null ? info.inputType : 0;
                    recorder.onFieldCleared(packageName, inputType);
                } catch (Exception e) {
                    Log.e(TAG, "Error notifying typing history on performEditorAction", e);
                }
            }
        }
    }
    ...
    public void commitCompletion(final CompletionInfo completionInfo) {
        ...
        mExpectedSelEnd = mExpectedSelStart;
        mComposingText.setLength(0);
        // Record suggestion/completion selection in typing history
        recordTypingHistory(text.toString());
        if (isConnected()) {
